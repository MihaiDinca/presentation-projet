...
class Game(Tkinter.Tk):
    ...
    def start_game(self):
        ...
        self.button10 =  Tkinter.Button(self.master, text='Comm \n ence \n le jeu  ', command = self.start_game_again, height = 2, width = 5)
        self.button10.grid(row=6, column=2)
        
        self.button11 =  Tkinter.Button(self.master, text='QUITER \n LE JEU', command = self.exit_game, height = 2, width = 5)
        self.button11.grid(row=6, column=4)

    def show_status_game(self):      
        y = self.compute_sums()
        if y == 1:
            self.label.config(text='X a gagne')
            return 1
        elif y == 2:
            self.label.config(text='O a gagne')
            return 2
        elif y !=1 or y!= 2:
            if self.nr == 10:
                self.label.config(text='persone n a pas gagne')
            return 3
    ...
    def switch_player(self):
        ...
        if self.player == 1:
            self.player = 10
            self.label.config(text='Allez  0')
            self.label.grid(row = 4)
        else:
            self.player = 1
            self.label.config(text='Allez  x')
            self.label.grid(row = 4)
